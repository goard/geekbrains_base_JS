...
function clickHandler(event) {
    ...
    let productTag = document.querySelectorAll('.product');
    if (productTag.length != 0) {
        productTag.forEach((key) => key.remove())
    }
...
function showCategory(category) {
    ...
    markup.forEach((key) => {
        productsTag.insertAdjacentHTML('beforebegin', key)
    })
...
function getProductMarkup(product) {
    const arr = Object.keys(product).map((key) => {
        return `<div class="product">
            <div>${product[key].name}</div>
            <img src="${product[key].imageUrl}" alt="">
            <div> ${product[key].price}</div >
            <a href="https://example.com/products/${product[key].id}">Подробнее</a>
        </div> `
    })
    return arr
}
